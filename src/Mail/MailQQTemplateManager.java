...
public class MailQQTemplateManager {
	...
	protected static int type = AccountUtil.qq_account;
	...
	public synchronized static MailTemplate getNextMail(String addr,int type)
	{
		synchronized(MailQQTemplateManager.class)
		{
			...
			{
				index = 0;
				SentTaskDB.setIndex("template_index", 0,type);
			}
			else
			{
				SentTaskDB.updateIndex("template_index", 1,type);
			}
			MailTemplate tmp = new MailTemplate(temps.get(index++));
			String str =tmp.getContent();
			try
			{
				str = str.replaceAll(hidden, "<TD>" + HiddenInfoManager.getNextInfo(type) + "</TD>");
			}
			catch(Exception e)
			{
				e.printStackTrace();
			...
			else
			{
				...
				{
					System.out.println(" ’º˛œ‰ " + addr + " base64Œª±‡¬Î ß∞‹");
				} 
			}
			tmp.setContent(str);
			return tmp;
	...
	{
		temps = MailDB.getTemplateFromDB(type);
	}
}
